...
st.title("MLB World Series Winners (Scraped)")
st.markdown(
  "This dashboard scrapes a historical table of MLB World Series winners and provides quick interactive exploration."
)
...
def load_data() -> pd.DataFrame:
  ...
  # Force column names (since the table has no proper header)
...
st.subheader("Raw table (first rows)")
...
st.subheader("Championships by Team (in selected range)")
titles = (
...
st.markdown(
  ...
  "- Converted `Season` to integer and removed non-numeric rows\n"
  "- Built an interactive filter and aggregated titles by winner"
)